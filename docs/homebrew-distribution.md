...
> 注意: CI が更新するのは **tap リポジトリ側の** `Casks/screen-painter.rb` です。
> このリポジトリ内の `Casks/screen-painter.rb` は初期テンプレートとしての位置づけで、
> CI 運用を始めたあとは tap リポジトリ側が最新(=ユーザーが実際にインストールする内容)になります。
> 手動運用の場合のみ、このリポジトリの Cask を正本として tap へコピーしてください。
